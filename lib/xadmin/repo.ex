defmodule XAdmin.Repo do
  @moduledoc false
  require Logger
  alias XAdmin.Utils
  alias XAdmin.Helpers
  alias XAdmin.Changeset
  import Ecto.Query
  require IEx
  ...
  def changeset_attributes_for(%Changeset{} = changeset, resource, params) do
    ...
    Enum.reduce insert_or_update_attributes_for(resource, params), {changeset, []},
      fn({cs, fun, field}, {acc, fields}) ->
        cs = XAdmin.Changeset.set_data cs, params
  ...
  def set_dependents(changeset, list) do
    ...
    XAdmin.Changeset.set_data changeset, fields
  end
  ...
  def insert(%XAdmin.Changeset{} = changeset) do
    case repo.insert changeset.changeset do
      {:ok, resource} ->
        case XAdmin.Schema.primary_key(resource) do
          nil -> resource
          key ->
