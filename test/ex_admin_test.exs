defmodule TestXAdmin.XAdmin.SimpleCustom do
  use XAdmin.Register
...
defmodule XAdminTest do
  ...
  @tag as_resource: %TestXAdmin.XAdmin.Simple{}
  test "action_button", %{defn: defn, conn: conn} do
    result = XAdmin.action_button(conn, defn, "Simple", :show, :edit, defn.actions, "17")
    assert result == [{"Edit Simple", [href: "/admin/simples/1/edit"]}]

    result = XAdmin.action_button(conn, defn, "Simple", :show, :new, defn.actions, "17")
    assert result == [{"New Simple", [href: "/admin/simples/new"]}]

    result = XAdmin.action_button(conn, defn, "Simple", :show, :delete, defn.actions, "17")
    ...
    result = XAdmin.action_button(conn, defn, "Simple", :index, :new, defn.actions, "17")
    assert result == [{"New Simple", [href: "/admin/simples/new"]}]

    result = XAdmin.action_button(conn, defn, "Simple", :edit, :new, defn.actions, "17")
    assert result == [{"New Simple", [href: "/admin/simples/new"]}]
  end
  ...
  test "default_resource_title_actions", %{defn: defn, conn: conn} do
    ...
    result = XAdmin.default_resource_title_actions(conn, defn)
  ...
  @tag as_resource: %TestXAdmin.XAdmin.SimpleCustom{}
