defmodule XAdmin.Index do
  @moduledoc """
  Override the default index page for an XAdmin resource

  By default, XAdmin renders the index table without any additional
  configuration. It renders each column in the model, except the id,
  inserted_at, and updated_at columns.

  ## Default Table Type

  XAdmin displays a selection checkbox column on the left with a batch
  ...
  For image fields, use the `image: true` option. For example:
      ...
      index do
        column :name
        column :image, [image: true, height: 100], &(XAdminDemo.Image.url({&1.image, &1}, :thumb))
      end
  ...
  By default, XAdmin will attempt to render a belongs_to association with a
          ...
          markup do
            div do
              a href: admin_resource_path(p, :show) do
                img(src: XAdminDemo.Image.url({p.image_file_name, p}, :thumb), height: 100)
              end
            end
  ...
  defmacro index(opts \\ [], do: contents) do
    quote location: :keep do
      def index_view(var!(conn), page, scope_counts) do
        ...
        import XAdmin.Form, except: [actions: 1]
        import XAdmin.ViewHelpers

        var!(columns, XAdmin.Show) = []
        var!(selectable_column, XAdmin.Index) = nil
        var!(actions, XAdmin.Index) = nil
        var!(cell, XAdmin.Index) = nil
        opts = unquote(opts)
        unquote(contents)

        selectable = case var!(selectable_column, XAdmin.Index) do
          nil -> false
          other -> other
        end
        actions = XAdmin.Index.get_index_actions(var!(conn).assigns.defn, var!(actions, XAdmin.Index))
        opts = Enum.into(opts, %{})
        |> Map.put(:column_list, var!(columns, XAdmin.Show) |> Enum.reverse)
        |> Map.put(:selectable_column, selectable)
        |> Map.put(:actions, actions)

        markup safe: true do
          XAdmin.Index.render_index_pages(var!(conn), page, scope_counts, var!(cell, XAdmin.Index), opts)
        end
      end
    end
  end
  ...
  defmacro actions(opts \\ []) do
    ...
    quote do
      var!(actions, XAdmin.Index) = unquote(opts)
    end
  end
  ...
  defmacro cell(fun) do
    quote do
      var!(cell, XAdmin.Index) = unquote(fun)
    end
  end
  ...
  defmacro selectable_column do
    quote do
      var!(selectable_column, XAdmin.Index) = true
    end
  end
    ...
    case conn.assigns.defn do
      ...
      %{__struct__: _} = defn ->
        columns = case defn.index_filters do
          ...
          [false] -> []
          [_] ->
            XAdmin.Filter.fields(conn.assigns.defn)
            |> Keyword.keys
        end
        |> case do
          [] ->
        ...
        markup safe: true do
          XAdmin.Index.render_index_pages(var!(conn), page, scope_counts, nil, opts)
        end
    end
  end
  ...
  def get_authorized_links(conn, links, _resource_model) do
    Enum.reduce links, [], fn(item, acc) ->
      if XAdmin.Utils.authorized_action?(conn, item), do: [item | acc], else: acc
    end
  end
end
