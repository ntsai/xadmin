defmodule XAdmin.Theme.ActiveAdmin.Index do
  @moduledoc false
    ...
    div ".blank_slate_container" do
      span ".blank_slate" do
        unless is_nil(conn.params["q"]) and is_nil(conn.params["scope"]) do
          ...
          if XAdmin.has_action?(conn, defn, :new) do
      ...
      div ".box-body.table-responsive.no-padding" do
        div ".paginated_collection" do
          table(".table-striped.index.table.index_table") do
            XAdmin.Table.table_head(columns, %{selectable: true, path_prefix: opts[:href],
  ...
  def do_footer(conn, opts) do
    ...
    div ".box-footer.clearfix" do
      ...
      |> XAdmin.Paginate.paginate(page.page_number, page.page_size, page.total_pages, opts[:count], opts[:name])
  ...
  def batch_action_form conn, enabled?, scopes, name, scope_counts, fun do
    ...
    scopes = unless Application.get_env(:xadmin, :scopes_index_page, true), do: [], else: scopes
      ...
      form "#collection_selection", action: "/admin/#{name}/batch_action", method: :post, "accept-charset": "UTF-8" do
        ...
        div ".box-header" do
          div ".table_tools" do
            ...
            if scopes != [] do
              current_scope = XAdmin.Query.get_scope scopes, conn.params["scope"]
                ...
                for {name, _opts} <- scopes do
                  ...
                  li ".scope.#{name}#{selected}" do
                    ...
                    a ".table_tools_button.btn-sm.btn.btn-default", href: href <> order_segment do
                      text XAdmin.Utils.humanize("#{name} ")
