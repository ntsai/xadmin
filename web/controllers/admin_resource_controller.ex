defmodule XAdmin.AdminResourceController do
  @moduledoc false
  use XAdmin.Web, :controller
  import XAdmin.Utils
  import XAdmin.ParamsToAtoms
  import XAdmin.Gettext
  alias XAdmin.Authorization
  ...
  defp render_403(conn) do
    ...
    |> put_layout(false)
    |> put_status(403)
    |> render(XAdmin.ErrorView, "403.html")
    |> halt
  end
  ...
  defp load_resource(conn, action, defn, resource_id) do
    ...
    |> XAdmin.Query.execute_query(repo, action, resource_id)
  ...
  def _handle_after_filter({conn, params, resource}, action, defn, [{name, opts} | t]) do
    ...
    if filter do
      case apply(defn.__struct__, name, [conn, params, resource, action]) do
        ...
        error ->
          raise XAdmin.RuntimeError, message: (gettext "invalid after_filter return:") <> " #{inspect error}"
      end
    else
  ...
  defp handle_plugs(conn, _action, defn) do
    case Application.get_env(:xadmin, :plug, []) do
  ...
  def index(conn, defn, params) do
    ...
    page = case conn.assigns[:page] do
      nil ->
        ...
        |> XAdmin.Query.execute_query(repo, :index, id)
    ...
    contents = if function_exported? model, :index_view, 3 do
      ...
      XAdmin.Index.default_index_view(conn, page, scope_counts)
    end
  ...
  def show(conn, defn, params) do
    ...
    contents = if function_exported? model, :show_view, 2 do
      ...
      XAdmin.Show.default_show_view(conn, resource)
    end
  ...
  defp do_form_view(conn, resource, params) do
    ...
    if function_exported? model, :form_view, 3 do
      ...
      XAdmin.Form.default_form_view conn, resource, params
    end
  end
  ...
  defp handle_changeset_error(conn, defn, changeset, params) do
    ...
    contents = do_form_view(conn, XAdmin.Changeset.get_data(changeset), params)
    render(conn, "admin.html", html: contents, filters: nil)
  end
  ...
  def create(conn, defn, params) do
    ...
    changeset = XAdmin.Repo.changeset(changeset_fn, resource, params[defn.resource_name])

    case XAdmin.Repo.insert(changeset) do
      {:error, changeset} ->
  ...
  def update(conn, defn, params) do
    ...
    case XAdmin.Repo.update(changeset) do
      {:error, changeset} ->
        conn |> handle_changeset_error(defn, changeset, params)
      resource ->
  ...
  def toggle_attr(conn, defn, %{attr_name: attr_name, attr_value: attr_value}) do
    ...
    render conn, "toggle_attr.js", attr_name: attr_name, attr_value: Map.get(resource, attr_name_atom), id: XAdmin.Schema.get_id(resource)
  end
  ...
  def destroy(conn, defn, params) do
    ...
    XAdmin.Repo.delete(resource, params[defn.resource_name])
  ...
  def batch_action(conn, defn, %{batch_action: "destroy"} = params) do
    ...
    type = case XAdmin.Schema.primary_key(resource_model) do
      nil -> :integer
  ...
  def csv(conn, defn, params) do
    ...
    |> XAdmin.Query.execute_query(repo, :csv, id)
    |> case  do
      [] -> []
      resources ->
        if function_exported? model, :build_csv, 1 do
          ...
          XAdmin.CSV.build_csv(resources)
        end
    end
  ...
  def repo, do: Application.get_env(:xadmin, :repo)
end
