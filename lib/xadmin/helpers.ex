...
defmodule XAdmin.Helpers do
  ...
  def build_link(contents, conn, _, resource, field_name) do
    case Map.get(resource, field_name) do
      nil -> contents
      res when is_map(res) ->
        if XAdmin.Utils.authorized_action? conn, :index, res.__struct__ do
  ...
  def model_name(resource) when is_atom(resource) do
    resource |> XAdmin.Utils.base_name |> Inflex.parameterize("_")
  end
  ...
  defp build_content_link(link?, conn, resource, contents) do
    if link? && XAdmin.Utils.authorized_action?(conn, :show) do
        ...
        cond do
          ...
          has_function?(struct_name, field, 1) ->
            try_function struct_name, resource, field, fn(_error) ->
              raise XAdmin.RuntimeError,
                message: "Could not call resource function #{:field} on #{struct_name}"
            end
          function_exported?(XAdmin.get_registered(resource.__struct__).__struct__, :display_name, 1) ->
            apply(XAdmin.get_registered(resource.__struct__).__struct__, :display_name, [resource])
          ...
          true ->
            case resource.__struct__.__schema__(:fields) do
              ...
              [id | _] ->
                Map.get resource, id
              _ ->
                raise XAdmin.RuntimeError, message: "Could not find field #{inspect field} in #{inspect resource}"
            end
        end
      _ ->
        raise XAdmin.RuntimeError, message: "Resource must be a struct"
    end
  end
  ...
  def display_name(resource) do
    defn = XAdmin.get_registered(resource.__struct__)
    cond do
      is_nil(defn) ->
