defmodule XAdminTest.ControllerTest do
  use TestXAdmin.ConnCase
  require Logger

  import TestXAdmin.TestHelpers
  alias TestXAdmin.{Noid, User, Product, Simple}
  ...
  test "batch selection for binary id" do
    noid = TestXAdmin.Noid.changeset(%TestXAdmin.Noid{}, %{name: "testing", description: "desc"})
    |> TestXAdmin.Repo.insert!
  ...
  test "restricted actions" do
    ...
    conn = post build_conn(), XAdmin.Utils.admin_resource_path(Restricted, :create)
    assert html_response(conn, 403) =~ ~r/Forbidden Request/
