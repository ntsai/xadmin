defmodule XAdmin.Register do
  @moduledoc """
  Allows registering a resource or a page to be displayed with XAdmin.

  For each model you wanted rendered by XAdmin, use the
  `register_resource` call. For each general page (like a dashboard),
  use the `register_page` call.

  To allow XAdmin to manage the resource with defaults, do not place
  ...
  Register the Survey.Answer model with all defaults.
      ...
      defmodule Survey.XAdmin.Answer do
        use XAdmin.Register
  ...
  * `changesets` - Defines custom changeset functions
  ...
  if File.dir?("/tmp") do
    ...
    @filename System.tmp_dir <> "/xadmin_registered"
  end

  import XAdmin.Utils
  import XAdmin.DslUtils

  defmacro __using__(_) do
    quote do
      use XAdmin.Index, except: [actions: 1]
      use XAdmin.Show
      use XAdmin.Form, except: [actions: 1]
      use XAdmin.CSV
  ...
  Once registered, XAdmin adds the resource to the administration
  ...
  * A CSV export link on the index page
  ...
  XAdmin will render an association using the following algorithm in the following order:
  ...
  * Use the 2nd field in the Model's schema
  ...
  defmacro register_resource(mod, [do: block]) do
    quote location: :keep do
      ...
      @name_column Module.get_attribute(__MODULE__, :name_column) || apply(XAdmin.Helpers, :get_name_field, [module])
      ...
      query_opts = case Module.get_attribute(__MODULE__, :query) do
        nil ->
          ...
          |> Enum.map(&(XAdmin.Register.build_query_association module, &1))
      ...
      |> XAdmin.Helpers.group_reduce_by_reverse

      actions =
        XAdmin.Register.get_action_items(Module.get_attribute(__MODULE__, :actions), @all_options)
        |> XAdmin.Register.custom_action_actions(Module.get_attribute(__MODULE__, :member_actions), module, :member_actions)
        |> XAdmin.Register.custom_action_actions(Module.get_attribute(__MODULE__, :collection_actions), module, :collection_actions)

      defstruct controller: @controller,
                controller_methods: Module.get_attribute(__MODULE__, :controller_methods),
                title_actions: &XAdmin.default_resource_title_actions/2,
                type: :resource,
      ...
      def run_query(repo, defn, action, id \\ nil) do
        ...
        |> XAdmin.Query.run_query(repo, defn, action, id, @query)
      end
      def run_query_counts(repo, defn, action, id \\ nil) do
        %__MODULE__{}
        |> Map.get(:resource_model)
        |> XAdmin.Query.run_query_counts(repo, defn, action, id, @query)
      end
      ...
      fun = quote do
        ...
        if type == :member_actions do
          fn id ->
            ...
            url = XAdmin.Utils.admin_resource_path(resource, :member, [name])
            XAdmin.ViewHelpers.action_item_link human_name, href: url, "data-method": :put
          end
        else
          fn id ->
            resource = module
            url = XAdmin.Utils.admin_resource_path(resource, :collection, [name])
            XAdmin.ViewHelpers.action_item_link human_name, href: url
          end
        end
      end
  ...
  * `plug` - Add a plug to the controller
  ...
  defmacro register_page(name, [do: block]) do
    quote location: :keep do
      import XAdmin.Register, except: [column: 1]
      use XAdmin.Page
      ...
      defstruct controller: Module.concat(Application.get_env(:xadmin, :project), AdminController),
                controller_methods: Module.get_attribute(__MODULE__, :controller_methods),
                type: :page,
                page_name: page_name,
                title_actions: &XAdmin.default_page_title_actions/2,
  ...
  * `:except` - Filters out actions in the except atom or list.
  ...
  ## Examples
      ...
      sidebar "XAdmin Demo", only: [:index, :show] do
        Phoenix.View.render XAdminDemo.AdminView, "sidebar_links.html", []
      end
  ...
  * `collection` - Add the collection for a belongs_to association
  ...
  defmacro column(name, opts \\ [], fun \\ nil) do
    quote do
      opts = XAdmin.DslUtils.fun_to_opts unquote(opts), unquote(fun)
      var!(columns, XAdmin.Show) = [{unquote(name), (opts)} | var!(columns, XAdmin.Show)]
    end
  end
  ...
  defmacro row(name, opts \\ [], fun \\ nil) do
    quote do
      opts = XAdmin.DslUtils.fun_to_opts unquote(opts), unquote(fun)
      var!(rows, XAdmin.Show) = [{unquote(name), opts} | var!(rows, XAdmin.Show)]
    end
  end
  ...
  * `action_items` macro will not remove any custom actions defined by the `action_item` macro.
  ...
  a backup and restore page.
      ...
      def restore_action(conn, params) do
        ...
        |> Controller.redirect(to: XAdmin.Utils.admin_resource_path(conn, :index))
      end
  ...
  * an optional label: "Button Label" (shown above)
  ...
  defp check_preload(field, key) do
    if Application.get_env :xadmin, key, true do
      field
    else
      nil
    end
