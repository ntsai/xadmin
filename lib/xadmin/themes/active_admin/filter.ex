...
defmodule XAdmin.Theme.ActiveAdmin.Filter do
  @moduledoc false
  require Logger
  require Ecto.Query
  import XAdmin.Utils
  import XAdmin.Gettext
  import XAdmin.Filter
  use Xain
  ...
  def build_field({name, %Ecto.Association.BelongsTo{related: assoc, owner_key: owner_key}}, q, defn) do
    ...
    if assoc.__schema__(:type, :name) do
      repo = Application.get_env :xadmin, :repo
      ...
      div ".filter_form_field.filter_select" do
        ...
        select "##{id}", [name: "q[#{owner_key}_eq]"] do
          option "Any", value: ""
          for r <- resources do
            id = XAdmin.Schema.get_id(r)
            name = XAdmin.Helpers.display_name(r)
  ...
  def build_field({name, type}, _q, _) do
    Logger.debug "XAdmin.Filter: unknown type: #{inspect type} for field: #{inspect name}"
    nil
  end
