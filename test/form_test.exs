defmodule XAdmin.FormTest do
  use ExUnit.Case, async: true
  alias TestXAdmin.{Simple, User, Role, Repo}
  use Xain
  ...
  test "build_control string" do
    res = XAdmin.Form.build_control(:string, %Simple{}, %{}, "simple", :name, "simple_name")
    assert res =~ "<input"
  ...
  test "build_control DateTime" do
    res = XAdmin.Form.build_control(Ecto.DateTime, %Simple{inserted_at: Ecto.DateTime.utc}, %{}, "simple", :inserted_at, "simple_inserted_at")
  ...
  test "build_control Date" do
    res = XAdmin.Form.build_control(Ecto.Date, %Simple{inserted_at: Ecto.DateTime.utc}, %{}, "simple", :inserted_at, "simple_inserted_at")
    select = Floki.find(res, "select[name='simple[inserted_at][year]']")
    refute select == []
  ...
  test "build_control Time" do
    res = XAdmin.Form.build_control(Ecto.Time, %Simple{inserted_at: Ecto.DateTime.utc}, %{}, "simple", :inserted_at, "simple_inserted_at")
    select = Floki.find(res, "select[name='simple[inserted_at][hour]']")
    refute select == []
  ...
  test "build_control :boolean" do
    res = XAdmin.Form.build_control(:boolean, %User{}, %{}, "user", :active, "user_active")
  ...
  test "build_item :input", %{conn: conn} do
    ...
    res = XAdmin.Form.build_item(conn, item, resource, "simple", nil)
  ...
  test "build_item :inputs as: :check_boxes collection", %{conn: conn} do
    ...
    res = XAdmin.Form.build_item(conn, item, %User{}, "user", nil)
  ...
  test "build_item :inputs  collection", %{conn: conn} do
    ...
    #   fun = fn(_p) ->
    #     [%{name: :number, opts: %{},
    #        resource: {TestXAdmin.PhoneNumber, TestXAdmin.ContactPhoneNumber},
    #        type: :input},
    #      %{name: :label,
    ...
    #   res = XAdmin.Form.build_item(conn, item, resource, "contact", nil)
    #   assert res == ""
    # end
end
