defmodule XAdmin.AdminAssociationController do
  @moduledoc false
  ...
  def index(conn, _defn, resource, %{"association_name" => association_name} = params) do
    ...
    page = XAdmin.Model.potential_associations_query(resource, defn_assoc.__struct__, assoc_name, params["keywords"])
    |> repo.paginate(params)

    results = page.entries
    |> Enum.map(fn(r) -> %{id: XAdmin.Schema.get_id(r), pretty_name: XAdmin.Helpers.display_name(r)} end)
  ...
  def add(conn, defn, resource, %{"association_name" => association_name, "selected_ids" => selected_ids} = params) do
    ...
    |> redirect(to: XAdmin.Utils.admin_resource_path(resource, :show))
  end
  ...
  defp repo, do: Application.get_env(:xadmin, :repo)
end
