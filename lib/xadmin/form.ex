defmodule XAdmin.Form do
  @moduledoc """
  Override the default new and edit form pages for an XAdmin resource.

  By default, XAdmin renders the form page without any additional
  ...
  XAdmin supports in-line creation of a has_many, through: (many-to-many)
  ...
  import XAdmin.ViewHelpers, only: [escape_javascript: 1]
  require IEx
  ...
  defmacro form(resource, [do: contents]) do
    quote location: :keep, bind_quoted: [resource: escape(resource), contents: escape(contents)] do
      import XAdmin.Index, only: []
      def form_view(var!(conn), unquote(resource) = var!(resource), var!(params) = params) do
        import XAdmin.Register, except: [actions: 1]

        var!(input_blocks, XAdmin.Form) = []
        var!(script_block, XAdmin.Form) = nil
        unquote(contents)
        items = var!(input_blocks, XAdmin.Form) |> Enum.reverse
        script_block = var!(script_block, XAdmin.Form)
      ...
      def get_blocks(var!(conn), unquote(resource) = var!(resource), var!(params) = _params) do
        ...
        var!(script_block, XAdmin.Form) = nil
        unquote(contents)
        var!(input_blocks, XAdmin.Form) |> Enum.reverse
      end
      ...
      def ajax_view(conn, params, resource, resources, block) do
        ...
        view = markup safe: true do
          XAdmin.Form.Fields.ajax_input_collection(resource, resources, model_name, field_name, params[:id1], params[:nested2], block, conn.params)
        end
  ...
  defmacro inputs(name, opts, do: block) do
    quote location: :keep do
      import Xain, except: [input: 1]
      # inputs_save = var!(inputs, XAdmin.Form)
      var!(inputs, XAdmin.Form) = []
      unquote(block)
      items = var!(inputs, XAdmin.Form) |> Enum.reverse
      # var!(inputs, XAdmin.Form) = inputs_save
      input_block = %{type: :inputs, name: unquote(name), inputs: items, opts: unquote(opts)}
      var!(input_blocks, XAdmin.Form) = [input_block | var!(input_blocks, XAdmin.Form)]
    end
  ...
  defmacro inputs(name, opts) do
    quote location: :keep do
      ...
      var!(inputs, XAdmin.Form) = [item | var!(inputs, XAdmin.Form)]
    end
  end
  ...
  defmacro actions(do: block) do
    quote do
      var!(inputs, XAdmin.Form) = []
      var!(items, XAdmin.Form) = []
      unquote(block)
      items = var!(items, XAdmin.Form) |> Enum.reverse
  ...
  defmacro content(items, opts \\ quote(do: [])) do
    quote do
      ...
      var!(items, XAdmin.Form) = [item | var!(items, XAdmin.Form)]
    end
  end
  ...
  defmacro javascript(do: block) do
    quote do
      var!(script_block, XAdmin.Form) = unquote(block)
    end
  end
      ...
      field ->
        case resource.__struct__.__schema__(:association, field) do
          %Ecto.Association.BelongsTo{cardinality: :one, queryable: assoc} ->
            collection = Application.get_env(:xadmin, :repo).all assoc
  ...
  def wrap_item_type(type, label, ext_name, contents, error, required) do
    XAdmin.theme.wrap_item_type(type, label, ext_name, contents, error, required)
  end
      ...
      {contents, html} = theme_module(conn, Form).build_inputs_has_many field_name,
        human_label, fn ->
          ...
          |> Enum.map(fn({res, inx}) ->
            ...
            res_id = XAdmin.Schema.get_id(res)
            markup do
              html
        ...
        {assoc_model, _} = assoc_model_tuple = XAdmin.Repo.get_assoc_model(resource, field_name)
  ...
  def build_control(type, resource, opts, model_name, field_name, ext_name) do
    ...
    value = XAdmin.Render.to_string(value)
